...
{
    nh.param("optimization/lambda1_smooth",lambda1_s_,-1.0);
    nh.param("optimization/lambda1_guide",lambda1_g_,-1.0);
    nh.param("optimization/lambda1_feasibility", lambda1_d_, -1.0);

    nh.param("optimization/lambda2_smooth", lambda2_s_, -1.0);
    nh.param("optimization/lambda2_feasibility", lambda2_d_, -1.0);
    nh.param("optimization/lambda2_fitness", lambda2_f_, -1.0);
...
void BsplineOptimizer::setEnvironment(const DspMap::Ptr &env)
{
    this->dsp_map_ = env;
}
...
std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
{
    ...
    double step_size = dsp_map_->getResolution() / ((init_points.col(0) - init_points.rightCols(1)).norm() / (init_points.cols() - 1)) / 2;
    int in_id, out_id;
...
void BsplineOptimizer::calcSmoothnessCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient, bool falg_use_jerk)
{
    cost = 0.0;
...
bool BsplineOptimizer::refine_optimize()
{
    ...
    double origin_lambda4 = lambda2_f_;
    bool flag_safe = true;
    int iter_count = 0;
    do
    {
...
void BsplineOptimizer::combineCostRebound(const double *x, double *grad, double &f_combine, const int n)
{
    ...
    double f_smoothness, f_guide, f_feasibility;
    
    Eigen::MatrixXd g_smothness = Eigen::MatrixXd::Zero(3, cps_.size);
    Eigen::MatrixXd g_guide = Eigen::MatrixXd::Zero(3, cps_.size);
    Eigen::MatrixXd g_feasibility = Eigen::MatrixXd::Zero(3, cps_.size);

    calcSmoothnessCost(cps_.points, f_smoothness, g_smothness);
    calcGuideCost(cps_.points, f_guide, g_guide);
    calcFeasibilityCost(cps_.points,f_feasibility,g_feasibility);

    f_combine = lambda1_s_ * f_smoothness + new_lambda1_g_ * f_guide + lambda1_d_ * f_feasibility;
    Eigen::MatrixXd grad_3D = lambda1_s_ * g_smothness + new_lambda1_g_ * g_guide + lambda1_d_ * g_feasibility;
...
void BsplineOptimizer::combineCostRefine(const double *x, double *grad, double &f_combine, const int n)
{
    ...
    f_combine = lambda2_s_ * f_smoothness + lambda2_f_ * f_fitness + lambda2_d_ * f_feasibility;
    // printf("origin %f %f %f %f\n", f_smoothness, f_fitness, f_feasibility, f_combine);

    Eigen::MatrixXd grad_3D = lambda2_s_ * g_smoothness + lambda2_f_ * g_fitness + lambda2_d_ * g_feasibility;
    memcpy(grad, grad_3D.data() + 3 * order_, n * sizeof(grad[0]));
}
